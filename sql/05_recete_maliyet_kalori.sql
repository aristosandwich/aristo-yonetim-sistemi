-- Reçete miktar, alış fiyatı, maliyet ve manuel kalori desteği — 30.08.2026
BEGIN;

ALTER TABLE public.receteler
  ADD COLUMN IF NOT EXISTS porsiyon_kalori numeric(10,2) NOT NULL DEFAULT 0;

ALTER TABLE public.receteler
  DROP CONSTRAINT IF EXISTS receteler_porsiyon_kalori_kontrol;
ALTER TABLE public.receteler
  ADD CONSTRAINT receteler_porsiyon_kalori_kontrol
  CHECK (porsiyon_kalori >= 0 AND porsiyon_kalori <= 100000);

CREATE OR REPLACE FUNCTION public.aristo_recete_satiri_guncelle_v1(
  p_recete_id bigint,
  p_malzeme_id bigint,
  p_eski_miktar numeric,
...
BEGIN
  ...
  END IF;
  IF p_yeni_miktar IS NULL OR p_yeni_miktar <= 0 OR p_yeni_miktar > 100000
    ...
    RAISE EXCEPTION USING ERRCODE='22023', MESSAGE='Miktar, fiyat, birim veya kalori geçersiz.';
  END IF;

  SELECT * INTO v_recete FROM public.receteler WHERE id=p_recete_id FOR UPDATE;
  IF NOT FOUND OR v_recete.gram IS DISTINCT FROM p_eski_miktar
  ...
  UPDATE public.receteler
    SET gram=p_yeni_miktar, porsiyon_kalori=p_yeni_kalori
...
REVOKE ALL ON FUNCTION public.aristo_recete_satiri_guncelle_v1(
  bigint,bigint,numeric,numeric,numeric,numeric,text,numeric,numeric,numeric,text
) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.aristo_recete_satiri_guncelle_v1(
  bigint,bigint,numeric,numeric,numeric,numeric,text,numeric,numeric,numeric,text
) TO authenticated;
...
SELECT
  to_regprocedure(
    'public.aristo_recete_satiri_guncelle_v1(bigint,bigint,numeric,numeric,numeric,numeric,text,numeric,numeric,numeric,text)'
  ) IS NOT NULL AS recete_duzenleme_hazir;
